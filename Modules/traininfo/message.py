...
def create_message(latest: tuple[TrainStatus, ...], previous: tuple[TrainStatus, ...], width: int = 300) -> list[str] | None:
    ...
    incident_to_another = [
        l for l in latest
        if (p := previous_dict.get(l.train)) and l.status != p.status
    ]
    new_incidents = [l for l in latest if l.train not in previous_dict]
    resolved_incidents = [p for p in previous if p.train not in {l.train for l in latest} and p.status != "🚋平常運転"]
    unchanged_incidents = [l for l in latest if (p := previous_dict.get(l.train)) and l.status == p.status != "🚋平常運転"]

    messages = []
    for r in incident_to_another:
        messages.append(f"{r.train} : {previous_dict[r.train].status}➡️{r.status}\n{r.detail}")
